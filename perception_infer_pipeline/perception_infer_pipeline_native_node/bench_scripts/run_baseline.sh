...
# --- Wait up to 20s for DeepStream to appear ---
echo "[INFO] Waiting for DeepStream PID (max 20s)..."
for i in {1..20}; do
  DS_PID=$(pgrep -f deepstream-app | head -n1 || true)
  if [[ -n "$DS_PID" ]]; then
    echo "[INFO] Found DeepStream PID: $DS_PID"
    break
  fi
  sleep 1
done
...
if [[ -z "${DS_PID:-}" ]]; then
  echo "[WARN] No DeepStream PID found after waiting 20s — skipping profiler attach."
  exit 0
fi
...
# PID-scoped perf
OUT_PROC="$LOG/perf_proc_${MODE}_${TAG}.csv"
OUT_CORE="$LOG/perf_core_${MODE}_${TAG}.csv"
