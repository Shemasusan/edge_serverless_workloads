...
bench_preflight() {
    ...
    # Redis — sink0 type=6 will crash DeepStream silently if Redis is down
    if ! redis-cli -h 127.0.0.1 -p 6379 ping &>/dev/null; then
        echo "[PREFLIGHT FAIL] Redis not reachable at 127.0.0.1:6379" \
             "— required by [sink0] msg-broker-conn-str in av_app.txt" >&2
        fail=1
    fi
...
bench_run_ncu() {
    ...
    echo ""
    echo "[PASS: ncu] Nsight Compute (slow)..."
    sudo ncu \
