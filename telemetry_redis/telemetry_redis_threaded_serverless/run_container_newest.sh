...
IMAGE="shemathomas/jsontelemetry_pi_perf_arm_new_stracetrial:latest"
...
if [ -f "$LOCAL_CSV" ]; then
  if [ ! -f "$CSVFILE" ]; then
    echo "Workload,Records,BatchSize,Thread_count,Latency_s,CPU_Util_Percent,Memory_MB,CPU_Cores_Used,Cores_List,LogFile,PerfFile" > "$CSVFILE"
  fi
  tail -n 1 "$LOCAL_CSV" >> "$CSVFILE"
fi
