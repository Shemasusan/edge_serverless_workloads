...
// Compute statistics from telemetry messages
...
void process_key(const std::string &k, const std::string &redis_host, int redis_port) {
    try {
        ...
        auto fill_stats = [](const Stats &st) {
            ...
            if (!std::isnan(st.mean))
                j["mean"] = st.mean;
            else
                j["mean"] = nullptr;

            if (!std::isnan(st.std_dev))
...
void process_all_files_sync(const std::string &redis_host, int redis_port, int thread_count, bool threaded) {
    try {
        std::string conn = "tcp://" + redis_host + ":" + std::to_string(redis_port);
        Redis redis(conn);
...
void process_all_files_http(const std::string &redis_host, int redis_port, int thread_count, bool threaded) {
    {
    ...
    process_all_files_sync(redis_host, redis_port, thread_count, threaded);
    {
        std::lock_guard<std::mutex> status_lock(g_status_mutex);
        g_status = "done";
    }
...
int main(int argc, char **argv) {
    ...
    int thread_count = std::stoi(get_env("THREAD_COUNT", "1"));
    ...
    if (mode.rfind("native", 0) == 0 || mode.rfind("container", 0) == 0) {
        ...
        svr.Post("/run", [&](const httplib::Request&, httplib::Response &res) {
            ...
            std::thread(process_all_files_http, redis_host, redis_port, thread_count, threaded).detach();
