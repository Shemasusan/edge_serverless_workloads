...
int main() {
    std::string mode = std::getenv("MODE") ? std::getenv("MODE") : "native";
...
/*    if (mode.find("serverless") == 0) {
        ...
        svr.Post("/run", [&](const httplib::Request &, httplib::Response &res) {
            std::thread([&, redis_host, redis_port, thread_count]() {
        ...
        svr.Get("/status", [&](const httplib::Request &, httplib::Response &res) {
            res.set_content(
                processing_done ? "{\"status\":\"done\"}" : "{\"status\":\"processing\"}",
                "application/json"
            );
        });

        std::cout << "[INFO] Serverless HTTP server running on port 8000\n";
        svr.listen("0.0.0.0", 8000);

    } else {*/
        // Native/container mode: existing logic
        fftw_initialize(thread_count);
