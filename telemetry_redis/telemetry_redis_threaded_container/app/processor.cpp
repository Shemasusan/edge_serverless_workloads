...
static Stats compute_stats(const std::vector<json> &messages, const std::string &key) {
    ...
    // choose FFT length as next power-of-two
    size_t M = 1;
    while (M < N) M <<= 1;
    if (M < 64) M = 64;
...
static void process_key(const std::string &k, const std::string &host, int port) {
    ...
    if (!redis_conn) {
        redis_conn = std::make_unique<Redis>("tcp://" + host + ":" + std::to_string(port));
    }
    ...
    try {
        ...
        try {
            ...
            while (start < s.size()) {
                ...
                if (end > start) {
                    try {
                        messages.push_back(json::parse(s.substr(start, end - start)));
                    } catch (...) {}
                }
                start = end + 1;
            }
        ...
        auto mk = [&](const Stats& st) {
            ...
            if (std::isfinite(st.std_dev) && st.std_dev > 0 &&
                std::isfinite(st.max) && std::isfinite(st.min))
...
int main() {
    ...
    // Redis isolation fix: scope processing to this run only
    const std::string run_id       = std::getenv("RUN_ID") ? std::getenv("RUN_ID") : "default";
    const std::string key_pattern  = "telemetry_" + run_id + "_*";
    ...
    if (mode == "serverless") {
        ...
        svr.Get("/status", [&](const httplib::Request &, httplib::Response &res) {
            res.set_content(
                busy ? "{\"status\":\"processing\"}" : "{\"status\":\"idle\"}",
                "application/json"
            );
        });
